...
class LinearSchedule(object):
    ...
    def update(self, t: int):
        ...
        range_of_eps = np.linspace(self.eps_begin, self.eps_end, int(self.nsteps+1))
        
        new_arr_2 = [round(i,1) for i in range_of_eps]
        
        # print(new_arr_2)
        # print(self.eps_begin, self.eps_end)
        # print(t)
        # print(new_arr_2[t])
        if t < self.nsteps:
            self.epsilon = range_of_eps[t]
        elif t >= self.nsteps:
            self.epsilon = range_of_eps[int(self.nsteps)]
        ##############################################################
        ######################## END YOUR CODE ############## ########
...
class LinearExploration(LinearSchedule):
    ...
    def get_action(self, best_action: int) -> int:
        ...
        generated_probability = np.random.rand()
        # print(self.env.num_actions())
        if generated_probability <= self.epsilon:
            action = random.randint(0,self.env.num_actions()-1)
            # print(action)
            return action
        elif generated_probability > self.epsilon:
            return best_action
        ##############################################################
        ######################## END YOUR CODE #######################
